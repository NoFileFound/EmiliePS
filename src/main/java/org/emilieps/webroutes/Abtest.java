...
public final class Abtest implements Response {
    ...
    public ResponseEntity<LinkedHashMap<String, Object>> SendExperimentList(@RequestBody ExperimentListModel body, @RequestHeader(value = "x-rpc-language", required = false) String lang) {
        ...
        try {
            ...
            if(!EncryptionLib.md5Encode(content).equals(body.app_sign)) {
                return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_ABTEST_AUTHENTICATION_FAILED, Application.getTranslations().get(lang, "retcode_abtest_authentication_failed"), null));
            }
        } catch (NumberFormatException ignored) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_ABTEST_AUTHENTICATION_FAILED, Application.getTranslations().get(lang, "retcode_abtest_authentication_failed"), null));
        }

        return ResponseEntity.ok(this.makeResponse(HttpRetcode.RETCODE_SUCC, "", MongodbLib.getSceneExperiments(Arrays.asList(body.scene_id.split(",")))));
    }
