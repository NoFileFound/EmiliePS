...
import static org.emilieps.data.HttpRetcode.RET_HTTP_BAD_REQUEST;
import jakarta.servlet.http.HttpServletRequest;
import java.util.LinkedHashMap;
import java.util.stream.Stream;
import org.emilieps.Application;
import org.emilieps.data.webserver.Response;
import org.emilieps.data.enums.webserver.CaptchaAction;
...
public final class Risky implements Response {
    ...
    public ResponseEntity<LinkedHashMap<String, Object>> SendCheck(@RequestBody RiskyCheckModel body, @RequestHeader(value = "x-rpc-game_biz", required = false) RegionType game_biz, @RequestHeader(value = "x-rpc-client_type", required = false) ClientType client_type, @RequestHeader(value = "x-rpc-language", required = false) String lang, HttpServletRequest request) {
        if(body == null || body.action_type == null || body.action_type.isEmpty() || body.api_name == null || body.api_name.isEmpty()) {
            return ResponseEntity.ok(this.makeResponse(RET_HTTP_BAD_REQUEST, Application.getTranslations().get(lang, "retcode_risky_invalid_request"), null));
        }
        ...
        if(Application.getHttpConfig().disable_mmt) {
        ...
        String riskId = EncryptionLib.md5Encode(EncryptionLib.generateRandomKey(32));
        String challengeHash = GeetestLib.generateChallenge();
		if(Application.getApplicationConfig().is_debug) {
			Application.getLogger().info(Application.getTranslations().get("console", "risky_new_captcha_generated", request.getRemoteAddr(), riskId));
		}
        ...
        return ResponseEntity.ok(this.makeResponse(RETCODE_SUCC, "OK", new LinkedHashMap<>() {{
            ...
            put("geetest", new LinkedHashMap<>() {{
                ...
                put("gt", Application.getHttpConfig().geetest.gt);
