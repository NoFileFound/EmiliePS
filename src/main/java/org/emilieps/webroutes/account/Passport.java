...
public final class Passport implements Response {
    ...
    public ResponseEntity<LinkedHashMap<String, Object>> SendSwitchStatus(String app_id, ClientType platform, @RequestHeader(value = "x-rpc-language", required = false) String lang) {
        if(!Application.getHttpConfig().app_ids.isEmpty() && !Application.getHttpConfig().app_ids.contains(app_id)) {
        ...
        return ResponseEntity.ok(this.makeResponse(HttpRetcode.RETCODE_SUCC, "OK",
            Map.of("switch_status_map", new LinkedHashMap<String, Object>() {{
                var props = Application.getHttpConfig().switch_status_map;
    ...
    public ResponseEntity<LinkedHashMap<String, Object>> SendLoginByMobileCaptcha(@RequestBody LoginByMobileCaptchaModel body, @RequestHeader(value = "x-rpc-device_id", required = false) String device_id, @RequestHeader(value = "x-rpc-language", required = false) String lang, HttpServletRequest request) {
        if(body.action_type == null || !body.action_type.equals("login_by_mobile_captcha") || body.area_code == null || body.area_code.isEmpty() || body.mobile == null || body.mobile.isEmpty() || body.captcha == null || body.captcha.isEmpty() || device_id == null) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_MA_PASSPORT_ILLEGAL_PARAMETER, Application.getTranslations().get(lang, "retcode_parameter_error"), null));
        }
        ...
        if(body.is_crypto) {
            ...
            if(body.area_code.isEmpty() || body.mobile.isEmpty()) {
                ...
                return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_MA_PASSPORT_SYSTEM_ERROR, Application.getTranslations().get(lang, "retcode_system_error"), null));
            }
        }
        ...
        if(myTicket == null) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_MA_PASSPORT_TICKET_NOT_EXIST, Application.getTranslations().get(lang, "retcode_system_error"), null));
        }

        if(!myTicket.getVerificationCode().equals(body.captcha)) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_MA_PASSPORT_CAPTCHA_MISMATCH, Application.getTranslations().get(lang, "retcode_verification_code_invalid"), null));
        }

        myTicket.delete();
        if(!myAccount.getApprovedDevices().contains(device_id)) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_MA_PASSPORT_ACCOUNT_NEW_DEVICE_DETECTED, Application.getTranslations().get(lang, "retcode_new_device_detected"), null));
        }

        String _actionType = "";
        for (String action : new String[]{"bind_realname", "modify_realname", "bind_realperson", "verify_realperson"}) {
            var _realPersonActionTicket = MongodbLib.findTicketByAccountId(myAccount.get_id(), action);
        ...
        return ResponseEntity.ok(this.makeResponse(HttpRetcode.RETCODE_SUCC, "OK", new LinkedHashMap<>() {{
            ...
            put("reactivate_info", new LinkedHashMap<String, Object>() {{
                ...
                put("ticket", (myAccount.getIsRequireReactivation()) ? MongodbLib.findTicketByAccountId(myAccount.get_id(), "reactivation").getId() : "");
            }});
            put("realname_info", new LinkedHashMap<String, Object>() {{
                put("required", myAccount.getIsRequireRealname());
                put("action_ticket", (myAccount.getIsRequireRealname() ? MongodbLib.findTicketByAccountId(myAccount.get_id(), final_actionType).getId() : ""));
    ...
    public ResponseEntity<LinkedHashMap<String, Object>> SendLoginByPassword(@RequestBody LoginByPasswordModel body, @RequestHeader(value = "x-rpc-device_id", required = false) String device_id, @RequestHeader(value = "x-rpc-language", required = false) String lang, HttpServletRequest request) {
        ...
        if(body.is_crypto != null && body.is_crypto) {
            body.account = EncryptionLib.decryptPassword(body.account);
            body.password = EncryptionLib.decryptPassword(body.password);
            if(body.password.isEmpty() || body.account.isEmpty()) {
                Application.getLogger().error(Application.getTranslations().get("console", "unable_to_decrypt_password", request.getRemoteAddr(), "app/loginByPassword"));
                return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_MA_PASSPORT_ILLEGAL_PARAMETER, Application.getTranslations().get(lang, "retcode_network_at_risk"), null));
            }
        }

        if(!body.account.matches("^(?:[A-Za-z0-9][A-Za-z0-9._]{2,19}|[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,})$")) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_MA_PASSPORT_ACCOUNT_FORMAT_ERROR, Application.getTranslations().get(lang, "retcode_account_format_error"), null));
        }

        if(!body.password.matches("^[!-~]{8,32}$")) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_MA_PASSPORT_PASSWORD_FORMAT_ERROR, Application.getTranslations().get(lang, "retcode_password_format_invalid"), null));
        }
        ...
        if(myAccount == null || !myAccount.getPassword().equals(EncryptionLib.sha256Encode(body.password))) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_MA_PASSPORT_ACCOUNT_NOT_EXIST, Application.getTranslations().get(lang, "retcode_login_account_not_found"), null));
        }
