...
import org.emilieps.utils.StringUtils;
...
public final class Panda implements Response {
    ...
    public ResponseEntity<LinkedHashMap<String, Object>> SendQRCodeConfirm(@RequestBody ConfirmModel body, @RequestHeader(value = "x-rpc-language", required = false) String lang) {
        if(!Application.getHttpConfig().enable_qrcode_login) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_QR_CODE_EXPIRED, Application.getTranslations().get(lang, "retcode_qr_code_disabled"), null));
        }
        ...
        if(myTicket == null || !myTicket.id.equals(body.ticket)) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_QR_CODE_EXPIRED, Application.getTranslations().get(lang, "retcode_qr_code_expired"), null));
        }
        ...
        var myAccount = MongodbLib.findAccountById(body.payload.get("raw").get("uid").asLong());
        if(myAccount == null) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_LOGIN_NETWORK_AT_RISK, Application.getTranslations().get(lang, "retcode_network_at_risk"), null));
        }
    ...
    public ResponseEntity<LinkedHashMap<String, Object>> SendQRCodeFetch(@RequestBody FetchModel body, @RequestHeader(value = "x-rpc-language", required = false) String lang) {
        ...
        if(body.app_id != ApplicationId.APP_GENSHIN && body.app_id != ApplicationId.APP_3NNN && body.app_id != ApplicationId.APP_CLOUDPLATFORM) {
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_INVALID_PARAMETER_ERROR, Application.getTranslations().get(lang, "retcode_system_error"), null));
        }
    ...
    public ResponseEntity<LinkedHashMap<String, Object>> SendQRCodeQuery(@RequestBody QueryModel body, @RequestHeader(value = "x-rpc-language", required = false) String lang) throws JsonProcessingException {
        ...
        if(System.currentTimeMillis() > (myTicket.getFetchedTime() + (myTicket.getTime() * 1000L))) {
            this.qrCodeInfoMap.remove(body.device);
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_QR_CODE_EXPIRED, Application.getTranslations().get(lang, "retcode_qr_code_expired"), null));
        }
        ...
        if(myTicket.getState() != QRCodeInfo.QRCodeState.Confirmed) {
            ...
            var myAccount = MongodbLib.findAccountByToken(myTicket.getPayload());
            if(myAccount == null) {
                return ResponseEntity.ok(this.makeResponse(HttpRetcode.RET_SYSTEM_ERROR, Application.getTranslations().get(lang, "retcode_system_error"), null));
            }
            ...
            return ResponseEntity.ok(this.makeResponse(HttpRetcode.RETCODE_SUCC, "OK", new LinkedHashMap<String, Object>() {{
                ...
                put("payload", new LinkedHashMap<String, Object>() {{
                    put("proto", "Account");
                    put("raw", JsonLib.toJson(new LinkedHashMap<String, Object>() {{
    ...
    public static class QRCodeInfo {
        ...
        public QRCodeInfo() {
            this.id = EncryptionLib.generateRandomKey(10);
            this.time = 300;
