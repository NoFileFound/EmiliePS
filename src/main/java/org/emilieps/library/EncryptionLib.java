...
public final class EncryptionLib {
    ...
    @Getter private static byte[] secretKey;
    @Getter private static final long secretKeySeed = Long.parseUnsignedLong("11468049314633205968");
    ...
    private static final String overseasMdkKey = "8e8a13e081aec9619ef8647bbc37da59";
    private static final String chinaMdkKey = "54c88f02dedb5fdfe997b40d325788ba";
    private static final String overseasComboKey = "7320c0c14ad8ce0e82fd8483f7d98435";
    private static final String chinaComboKey = "71f35d945cf97f8f202cef35ae6aa7ed";
    ...
    public static void loadEncryptionKeys() {
        if(Application.getApplicationConfig().enable_encryption) {
            try {
                ...
                secretKey = Files.readAllBytes(Paths.get("resources/dispatch/secretKey.bin"));
                ...
                Application.getLogger().info(Application.getTranslations().get("console", "rsaloadedkeys"));
            } catch (Exception ignored) {
                Application.getLogger().error(Application.getTranslations().get("console", "rsaloadedkeysfailed"));
                System.exit(1);
            }
        } else {
            dispatchSeed = new byte[0];
            dispatchKey = new byte[0];
            dispatchSecurityKey = new byte[0];
            passwordDecryptionKey = new byte[0];
            identityDecryptionKey = new byte[0];
            secretKey = new byte[0];
            try {
                dispatchSignatureKey = KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(new byte[0]));
            } catch (Exception ignored) {
    ...
    /**
     * Encodes a string into sha256.
     * @param plainText The given text.
     * @return An sha256 string.
     */
