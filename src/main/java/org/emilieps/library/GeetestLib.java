...
public final class GeetestLib {
    ...
    public static String generateChallenge() {
        try {
            HttpURLConnection conn = (HttpURLConnection) new URL("http://api.geetest.com/register.php?" + "gt=" + URLEncoder.encode(Application.getHttpConfig().geetest.gt, StandardCharsets.UTF_8) + "&json_format=1").openConnection();
            ...
            try (InputStream in = conn.getInputStream()) {
                ...
                String challenge = Objects.requireNonNull(JsonLib.parseJsonSafe(res)).get("challenge").asText();
                if (challenge.length() == 32) {
                    return EncryptionLib.md5Encode(challenge + Application.getHttpConfig().geetest.private_key);
                }
                return "";
            }
        } catch (Exception e) {
    ...
    public static Boolean checkVerifiedChallenge(String risky) {
        if(Application.getHttpConfig().disable_mmt) return true;
