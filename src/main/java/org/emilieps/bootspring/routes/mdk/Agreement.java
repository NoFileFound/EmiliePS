...
import static org.emilieps.bootspring.data.HttpRetcode.RET_PARAMETER_ERROR;
        ...
        if(!body.uid.isEmpty() && !body.token.isEmpty()) {
            ...
            if(body.operation.equals("ACCEPT")) {
				...
				if(Application.getPropertiesInfo().is_debug) {
					Application.getLogger().info(Application.getTranslationManager().get("console", "user_agreed_marketing_agreements", account.get_id(), agreement.agreement_id, agreement.agreement_version));
				}
            } else {
                account.getDeniedAgreements().add(agreement.agreement_id + "_" + agreement.agreement_version);
				if(Application.getPropertiesInfo().is_debug) {
					Application.getLogger().info(Application.getTranslationManager().get("console", "user_denied_marketing_agreements", account.get_id(), agreement.agreement_id, agreement.agreement_version));
				}
            }
